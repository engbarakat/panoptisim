...
with open('voli.txt') as inf:
  for line in inf:
    parts = line.split("\t") # split line into parts
    for y in parts:
        plistvoli.append(float(y))
with open('tvol.txt') as inf:
  for line in inf:
    parts = line.split("\t") # split line into parts
    for y in parts:
        plisttvol.append(float(y))
        
with open('ivolt.txt') as inf:
  for line in inf:
    parts = line.split("\t") # split line into parts
    for y in parts:
        plistivolt.append(float(y))
        
with open('evolf.txt') as inf:
  for line in inf:
    parts = line.split("\t") # split line into parts
    for y in parts:
...
pl.plot(swlist, plistvol, '-g', label = 'Vlan RAND',linewidth=3.0)
...
pl.title("Various Strategies on VLAN 256",size=50)
