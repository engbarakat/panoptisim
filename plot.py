...
with open('voli512.txt') as inf:
  for line in inf:
    parts = line.split("\t") # split line into parts
    for y in parts:
        plistvoli.append(float(y))
with open('tvol512.txt') as inf:
  for line in inf:
    parts = line.split("\t") # split line into parts
    for y in parts:
        plisttvol.append(float(y))
        
with open('ivolt512.txt') as inf:
  for line in inf:
    parts = line.split("\t") # split line into parts
    for y in parts:
        plistivolt.append(float(y))
        
with open('evolf512.txt') as inf:
  for line in inf:
    parts = line.split("\t") # split line into parts
    for y in parts:
...
pl.title("Various Strategies on VLAN 512",size=50)
